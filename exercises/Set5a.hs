...
getName (PersonUndefined age name) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (PersonUndefined age name) = age
...
get :: UpDown -> Int
get (UpCounter val) = val
get (DownCounter val) = val
...
fromBin :: Bin -> Int
fromBin = fromBin' 1
  where fromBin' bit End = 0
        fromBin' bit (I b) = bit + fromBin' (bit+bit) b
        fromBin' bit (O b) = fromBin' (bit+bit) b

toBin :: Int -> Bin
-- toBin n
--   | n == 0 = O End
--   | otherwise = inc (toBin (n-1))
toBin 0 = (O End)
toBin n = toBin' n
  where toBin' 0 = End
        toBin' n
          | n `mod` 2 == 0 = O (toBin' (div n 2)) 
