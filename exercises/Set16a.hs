...
sumIsLength input output = length input === (sum.snd.unzip)output
...
inputInOutput input output = forAll (elements input) (\x -> elem x (fst.unzip$output))
...
outputInInput input output = forAll (elements output) check
  where check (x,n) = length (filter (==x) input) === n
...
frequenciesProp freq (NonEmpty input) = sumIsLength input output .&&. inputInOutput input output .&&. outputInInput input output
...
instance Arbitrary Arg where
  arbitrary = do
    v <- elements "abcxyz"
    n <- choose (0,10)
    elements [(Number n), (Variable v)]

instance Arbitrary Expression where
   arbitrary = do
    arg1 <- arbitrary::Gen Arg
    arg2 <- arbitrary::Gen Arg
    elements [(Plus arg1 arg2), (Minus arg1 arg2)]
