...
blendColor (Color a b c) (Color d e f) = Color (avg a d) (avg b e) (avg c f)
...
rectangle x0 y0 w h = Shape f
  where f (Coord x y) | x >= x0 && x < x0 + w && y >= y0 && y  < y0 + h = True
                      | otherwise = False
-- rectangle x0 y0 w h = Shape f
...
union (Shape f1) (Shape f2) = Shape f
  where f coord = f1 coord || f2 coord

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = Shape f
  where f coord | f1 coord && not (f2 coord) = True
                | otherwise = False
...
paintSolid color (Shape f) (Picture base) = 
  Picture g where g coord | f coord = color
...
paint (Picture pat) (Shape f) (Picture base) = Picture g
  where g coord | f coord = pat coord
                | otherwise = base coord
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill color) p = Picture (\coord -> color)
...
instance Transform Flip where
  apply FlipXY = flipXY
  apply FlipX = flipX
    where flipX (Picture f) = Picture (f.(\(Coord x y) -> (Coord (-x) y)))
  apply FlipY = flipY
    where flipY (Picture f) = Picture (\(Coord x y) -> f(Coord x (-y)))
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture base) = Picture f 
    where avg (Color x0 y0 z0) (Color x1 y1 z1) (Color x2 y2 z2) (Color x3 y3 z3) (Color x4 y4 z4)
            = Color (div (sum [x0, x1, x2, x3, x4]) 5) (div (sum[y0, y1, y2, y3, y4]) 5) (div (sum [z0,z1,z2,z3,z4]) 5)
          f (Coord x y) = avg (base (Coord x y)) col1 col2 col3 col4
            where col1 = base(Coord (x+1) y)
                  col2 = base(Coord (x-1) y)
                  col3 = base(Coord x (y-1))
                  col4 = base(Coord x (y+1))
