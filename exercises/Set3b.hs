...
sums' count 0 sum = []
sums' count i sum = sum : sums' (count+1) (i-1) (sum+count)
...
mylast :: a -> [a] -> a
mylast def [] = def
mylast def (last:[]) = last
mylast def (head:xs) = mylast def xs
-- model answer
...
indexDefault (head:tail) 0 def = head
indexDefault (head:tail) i def = indexDefault tail (i-1) def
...
sumsOf' :: Int->[Int]->[Int]
sumsOf' _ [] = []
sumsOf' soFar (first:others) = (soFar+first): sumsOf' (soFar+first) others
...
merge [] ys = ys
merge xs [] = xs
merge (xsFirst:xsOthers) (ysFirst: ysOthers)
    | xsFirst < ysFirst = xsFirst: merge xsOthers (ysFirst:ysOthers)
    | otherwise         = ysFirst : merge (xsFirst:xsOthers) ysOthers
------------------------------------------------------------------------------
...
mymaximum bigger initial (first:others)
    |bigger first later = first
    |otherwise          = later
    where later = mymaximum bigger initial others
-- model answer
...
map2 f as [] = []
map2 f [] bs = []
map2 f (asFirst:as) (bsFirst:bs) = (f asFirst bsFirst): map2 f as bs
