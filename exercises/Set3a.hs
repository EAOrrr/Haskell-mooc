...
mapMaybe f (Just thing) = Just(f thing)
...
capitalize = unwords . (map (\string -> (toUpper.head) string: tail string)) .words 
...
powers :: Int -> Int -> [Int]
powers k max = go 1 k max
    where go n k max
            |n > max   = []
            |otherwise = n: (go (n * k) k max)
-- model answer:
-- powers :: Int -> Int -> [Int]
...
whileRight check x = case (check x) of (Left left) -> left
                                       (Right right) -> whileRight check right
...
-- joinToLength n lst= [elem| elem <- [first++second| first <- lst, second <- lst, first /= second ], ((==n).length)(elem)]
-- model answer
joinToLength :: Int -> [String] -> [String]
...
sumRights [] = 0
sumRights ((Right k):xs) = k + sumRights xs
...
multiCompose [] = id
multiCompose (f:xs) = f.(multiCompose xs)
...
multiApp f gs elem = f (multiApp' gs elem)
    where multiApp' [] elem = []
          multiApp' (g:xs) elem = (g elem) : (multiApp' xs elem)
-- answer to the challenge  -- elegant !!!
...
interpreter' x y [] = []
interpreter' x y (command: commands) = case command of "up"     -> interpreter' x (y+1) commands
                                                       "down"   -> interpreter' x (y-1) commands
                                                       "left"   -> interpreter' (x-1) y commands
                                                       "right"  -> interpreter' (x+1) y commands
                                                       "printX" -> (show x): interpreter' x y commands
                                                       "printY" -> (show y): interpreter' x y commands
