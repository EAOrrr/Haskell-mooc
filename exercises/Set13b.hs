...
ifM opBool opThen opElse = do
  val <- opBool
  if val then opThen else opElse
...
visit maze place = do
  state <- get
  if elem place state then return () else do  
    put (place:state)
    let Just xs = lookup place maze
    -- mapM_ (\x -> if elem x state then return() else visit maze x) xs
    mapM_ (\x -> visit maze x) xs

-- visit :: [(String,[String])] -> String -> State [String] ()
-- visit maze place = do visited <- get
--                       when (not (elem place visited)) $ do modify (place:)
--                                                            forM_ neighbours (visit maze)
--   where neighbours = case lookup place maze of Nothing -> []
                                              --  Just places -> places
...
path maze place1 place2 = let (_, p) = runState (visit maze place1) []
                          in elem place2 p
...
findSum2 ks ns = [(i,j,k)| i <- ks, j <- ks, k <- ns, i+j == k]
...
f2 acc x = do
  occurrence <- get
  if elem x occurrence then return acc
                       else put(x:occurrence) >> return (acc+x)
...
instance Functor SL where
  -- implement fmap
  fmap g (SL f) = SL h
    where h state0 = let (a, state1, msgs) = runSL (SL f) state0
                     in (g a, state1, msgs)
...
instance Monad SL where
  -- implement return and >>=
  return x = SL(\s -> (x, s, []))
  (>>=) (SL f) parameterized = SL h
    where h state0 = let (a, state1, msgs) = runSL (SL f) state0
                         (a', state1', msgs') = runSL (parameterized a) state1
                     in (a', state1', msgs++msgs')
