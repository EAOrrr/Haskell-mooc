...
palindromify s = if isPalindrome s then s else palindromify (init(tail s))
...
safeIndex xs i =  if i >= (length xs) || i < 0 then Nothing else Just (xs !! i) 
...
eitherDiv x y = if y == 0 then Left (show x ++ "/" ++ show y) else Right (div x y)
