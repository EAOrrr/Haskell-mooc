...
-- allEqual :: [a] -> Bool -- No instance for (Eq a) arising from a use of ‘==’
allEqual [] = True
allEqual (x:[]) = True
allEqual (first:second:xs) = (first == second) && allEqual (second:xs)
...
longest (x:xs) = go x xs
    where go biggest [] = biggest
          go biggest (x:xs)
            | length x < length biggest =  go biggest xs
            | length x > length biggest =  go x       xs
            | x > biggest               =  go biggest xs
            | otherwise                 =  go x       xs
...
incrementKey key [] = []
incrementKey key ((k,v): xs) = case key == k of 
                                True -> (k, v+1):incrementKey key xs
                                False -> (k, v): incrementKey key xs
...
winner scores player1 player2
    = case (Map.findWithDefault 0 player1 scores) < (Map.findWithDefault 0 player2 scores) of
        False -> player1
        True -> player2
...
freq [] = Map.empty
freq (x:xs) = Map.alter (+1)
-- freqs xs = foldr f Map.empty xs
--     where f x = Map.insertWith (+) x 1
...
transfer from to amount bank = 
    case Map.lookup from bank of
        Nothing -> bank
        Just fromSum -> case fromSum - amount >= 0 && amount > 0 of
                            False -> bank
                            True  -> case  Map.lookup to bank of 
                                        Nothing -> bank
                                        Just toSum -> Map.insert from (fromSum - amount) (Map.insert to (toSum + amount) bank)
...
maxIndex arr = (fst.last) $sortBy f (assocs arr)
