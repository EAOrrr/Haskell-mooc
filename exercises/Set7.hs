...
member v (Set xs)= elem v xs

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add v (Set xs) = Set (go v xs)
  where go v [] = [v]
        go v (x:xs)
          | v > x = x : (go v xs)
          | v < x = v:x:xs
          | otherwise = x:xs
-- add v (Set xs) = Set((sort.nub)$v:xs)
...
data State = Start | AfterEgg | AfterFlour | AfterSugar | AfterFlourAndSugar | AfterMix | Finished | Error
  deriving (Eq,Show)
...
step AfterFlour AddSugar = AfterFlourAndSugar
step AfterSugar AddFlour = AfterFlourAndSugar
step AfterFlourAndSugar Mix = AfterMix
...
average xs = (eSum xs) / (fromIntegral (length xs))
  where eSum (x :| []) = x
        eSum (x :| xs) = x + sum xs
...
instance Ord a => Semigroup (Set a) where 
  ...
  Set [] <> Set b = Set b
  Set a <> Set (b:bs)
    | member b (Set a) = Set a <> Set bs
    | otherwise        = add b (Set a <> Set bs)
-- Elegant !!!
-- instance Ord a => Semigroup (Set a) where
...
passwordAllowed pw (MinimumLength k) = (length pw) >= k
passwordAllowed pw (ContainsSome xs) = foldr (\x y -> (elem x pw) || y) False xs
passwordAllowed pw (DoesNotContain xs) = not (passwordAllowed pw (ContainsSome xs))
...
--     ==> "(3*(1+1))"
--
data Arithmetic = Arithmetic Integer | Add Arithmetic Arithmetic | Mul Arithmetic Arithmetic
  deriving Show
