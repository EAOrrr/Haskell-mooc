...
instance Functor TwoList where
  ...
  fmap f (TwoNode x y t) = TwoNode (f x) (f y) (fmap f t)
...
count i items = foldr f 0 items
  where f item n = if item == i then (n+1) else n
...
inBoth t1 t2 = foldr f [] t1
  where f t xs = if elem t t2 then t:xs else xs
...
instance Foldable List where
  foldr f y Empty = y
  foldr f y (LNode x xs) = f x (foldr f y xs)
...
instance Foldable TwoList where
  foldr f z TwoEmpty = z
  foldr f z (TwoNode x y t) = f x (f y (foldr f z t))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node v l r) = (sumTree l)<>v<>(sumTree r)
